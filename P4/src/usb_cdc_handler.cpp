// =============================================================================
// usb_cdc_handler.cpp — USB Host CDC-ACM (P4 talks to DaisyPod3 via USB-C)
// ESP32-P4 acts as USB Host on its OTG port.
// libDaisy presents its standard internal USB CDC device (0483:5740).
// =============================================================================
...
// libDaisy internal USB CDC identifiers.
#define DAISY_USB_VID  0x0483
#define DAISY_USB_PID  0x5740
...
#define CDC_TX_QUEUE_DEPTH 48
#define CDC_TX_MAX_LEN     1040  // RED808 header + binary command/sample chunk
...
// Called by CDC driver when data arrives from DaisyPod3
...
static void usb_cdc_tx_task(void* arg) {
    // Static storage is intentional: a framed WAV packet is ~1 KB. Keeping it
    // on this task's small stack could overflow before the first USB hello was
    // transmitted, leaving the UI permanently at USB WAIT.
    static CdcTxPkt pkt;
    while (true) {
...
static void finalize_connection(const char* method, bool assert_dtr) {
    ...
    // Assert conventional terminal control lines. libDaisy does not use these
    // lines for reset, but accepting them keeps the CDC class host-compatible.
...
// =============================================================================
// PROCESS — call from loop(); tries to open DaisyPod3 when not connected
// =============================================================================
void usb_cdc_process(void) {
    ...
    // timeouts every retry and starving the local controller.
    ...
    // DaisyPod3 exposes one libDaisy CDC function with the fixed VID/PID below.
    // Probe its small interface range only; wildcard
    // probing could attach to an unrelated CDC device on the P4 host port.
    esp_err_t err;

    const uint8_t ifaces[] = {0, 1, 2, 3};
    for (uint8_t iface : ifaces) {
        err = cdc_acm_host_open(DAISY_USB_VID, DAISY_USB_PID, iface,
    ...
    if (s_open_attempts % 5 == 1) {
        char buf[128];
        snprintf(buf, sizeof(buf), "[USB-CDC] No DaisyPod3 (attempt #%d)\n", s_open_attempts);
        P4_LOG_PRINT(buf);
